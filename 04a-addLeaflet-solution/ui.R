...
shinydashboard::dashboardPage(
      ...
      shinydashboard::dashboardBody(
        tags$head(tags$link(rel = "stylesheet", type = "text/css", href = "mycss.css")),
            shinydashboard::tabItem("xx",                                                  ##CHG: add tabItems 
                                    shiny::tabsetPanel(                                    ##CHG: add tabsetPanel 
                                        shiny::tabPanel("Table",
                                                        fluidRow(tableOutput("TableOut.byNetwork"),
                                                                 plotlyOutput("PlotOut.byNetwork"))
                                        ),
                                        shiny::tabPanel("Leaflet",                         ##CHG: add Leaflet 
                                                        fluidRow(
                                                          selectInput("KPI.In",            ##CHG: add KPI selection
